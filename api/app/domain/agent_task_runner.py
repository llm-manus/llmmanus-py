...
class AgentTaskRunner(TaskRunner):
    ...
    def __init__(
            ...
            file_repository: FileRepository, # 会话数据仓库
    ...
    async def _pop_event(cls, task: Task) -> Event:
        ...
        if event_str is None:
            logger.warning(f"AgentTaskRunner接受到空消息")
            return
    ...
    async def _sync_message_attachments_to_sandbox(self, event: MessageEvent) -> None:
        ...
        try:
            ...
            logger.exception(f"AgentTaskRunner同步消息列表附件到沙箱失败：{str(e)}")

    async def _sync_file_to_storage(self, filepath: str) -> File:
        """将沙箱中指定的文件路径数据同步到存储桶中"""
        try:
            # 1.根据文件路径从会话中查找文件数据
            file = await self._session_repository.get_file_by_path(self._session_id, filepath)
    ...
    async def _run_flow(self, message: Message) -> AsyncGenerator[BaseEvent, None]:
        ...
        if not message.message:
            logger.warning(f"AgentTaskRunner接受了一条空消息")
        ...
        try:
            ...
            while not await task.input_stream.is_empty():
                ...
                if isinstance(event, MessageEvent):
                    ...
                    logger.info(f"AgentTaskRunner接受到新消息：{message[:50]}...")
                ...
                async for event in self._run_flow(message_obj):
                    ...
                    if isinstance(event, TitleEvent):
                        ...
                        await self._session_repository.increase_unread_message_count(self._session_id)
