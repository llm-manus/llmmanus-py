...
class BrowserTool(BaseTool):
    ...
    @tool(
        name='browser_view',
        description="查看当前浏览器页面内容，用于确定已打开页面的最新状态。",
        parameters={},
        required=[]
    )
    ...
    @tool(
        ...
        description="将浏览器导航至指定网址，当续页访问新页面时使用。",
        parameters={
            "url": {
                "type": "string",
    ...
    @tool(
        ...
        description="重新启动浏览器并导航至指定URL，当续页重置浏览器时使用",
        parameters={
            "url": {
                "type": "string",
                "description": "要访问的完整URL，必须包含协议前缀(例如https://)",
    ...
    @tool(
        ...
        parameters={
            "key": {
                "type": "string",
                "description": "要模拟的案件名称(例如：Enter、Tab、ArrowUp)，支持组合键(例如：Control+Enter)",
            },
        },
        required=["key"],
    )
    ...
    async def browser_select_option(self, index: int, option: str) -> ToolResult:
        """传递索引+下拉元素选项序号执行选择"""
    ...
    @tool(
        ...
        parameters={
            "to_down": {
                "type": "boolean",
                "description": "(可选)是否直接滚动到页面下部，而非向上滚动一屏",
            },
        },
        required=[],
    )
    async def browser_scroll_down(self, to_down: bool) -> ToolResult:
    ...
    @tool(
        ...
        description="查看浏览器控制台输出，用于检查JavaScript日志或调试页错误。",
        parameters={
            "max_lines": {
                "type": "integer",
