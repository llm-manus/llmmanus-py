...
class SearchTool(BaseTool):
    ...
    @tool(
        ...
        parameters={
            "query": {
                "type": "string",
                "description": "针对搜索引擎优化的查询字符串。请提取问题中的核心实体和关键词（3-5个），避免使用完整的自然语言问句（例如将'今天北京的天气怎么样'改为'北京 天气'）。"
            },
            "date_range": {
                "type": "string",
                "enum": ["all", "past_hour", "past_day", "past_week", "past_month", "past_year"],
                "description": "（可选）搜索结果的事件范围过滤。当用户询问特定时效性的新闻或事件时（如'昨天'、'上周'），必须指定此采纳数。默认为'all'。"
            },
        },
        required=["query"],
    )
    async def search_web(self, query: str, date_range: Optional[str] = None) -> ToolResult[SearchResults]:
        """调用搜索引擎获取搜索结果后返回"""
        return await self.search_engine.invoke(query, date_range=date_range)
