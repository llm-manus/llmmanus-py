...
from app.domain.services.prompts.planner import PLANNER_SYSTEM_PROMPT, CREATE_PLANNER_PROMPT, UPDATE_PLANNER_PROMPT
...
3. ReActAgent执行完每一个子步骤之后，需要将子步骤结果+Plan传递PlannerAgent让其更新计划/Plan;
4. 循环取出规划中的子步骤，让ReActAgent执行，依次迭代;
5. ...
6. 直到所有子任务/步骤都完成，这时候将子步骤的所有结果汇总尽享总结(ReActAgent);

PlannerAgent:
- 功能: 将用户的需求拆解成多个子任务+根据已完成的子任务更新规划
- 提示词: 创建规划的prompt、更新规则的prompt
