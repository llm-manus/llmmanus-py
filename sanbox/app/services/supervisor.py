...
class SupervisorService:
    ...
    def __init__(self) -> None:
        ...
        # 1.连接Supervisor配置
        self.rpc_url = "/tmp/supervisor.so  ck"
        self._connect_rpc()
        ...
        self._expand_enabled = True  # 是否自动保活(每调页一次接口就增加时间)
    ...
    def _setup_timer(self, minutes: int) -> None:
        ...
        try:
            ...
            if hasattr(self, "shutdown_timer") and self.shutdown_time:
    ...
    async def cancel_timeout(self) -> SupervisorTimeout:
        ...
        # 3.同步检查是否存在定时器
        if hasattr(self, "shutdown_task") and self.shutdown_timer:
            self.shutdown_timer.cancel()
            self.shutdown_task = None
